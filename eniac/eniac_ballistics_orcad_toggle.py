# eniac_ballistics_orcad_toggle.py
# --------------------------------------------------------------
# ENIAC Ballistics – OrCAD Layout (toggle + routing clearance)
# - Single Play/Pause toggle button (and Space key) — Pause button removed.
# - Reset (R), Step (N), Fullscreen (F11) kept.
# - Manhattan routing now uses clearance channels so vertical segments
#   do not stick to block edges; they move outward first then route.
# --------------------------------------------------------------
...
CLEAR = 14  # clearance away from block edge before turning
FONT_NAME = "consolas"
...
class Wire:
    """Manhattan router with edge clearance and obstacle avoidance."""
    ...
    def path(self, obstacles):
        ...
        # move outward from each edge by CLEAR before routing
        if self.src_side == "right": sx_clear = (sx + CLEAR, sy)
        elif self.src_side == "left": sx_clear = (sx - CLEAR, sy)
        elif self.src_side == "top": sx_clear = (sx, sy - CLEAR)
        else: sx_clear = (sx, sy + CLEAR)

        if self.dst_side == "left": dx_clear = (dx - CLEAR, dy)
        elif self.dst_side == "right": dx_clear = (dx + CLEAR, dy)
        elif self.dst_side == "top": dx_clear = (dx, dy - CLEAR)
        else: dx_clear = (dx, dy + CLEAR)

        # Primary: H-V via middle channel between clear points
        mid_x = (sx_clear[0] + dx_clear[0]) // 2
        hv = [ (sx, sy), sx_clear, (mid_x, sx_clear[1]), (mid_x, dx_clear[1]), dx_clear, (dx, dy) ]
        if not path_hits_any(hv, obstacles): return hv

        # Secondary: V-H
        mid_y = (sx_clear[1] + dx_clear[1]) // 2
        vh = [ (sx, sy), sx_clear, (sx_clear[0], mid_y), (dx_clear[0], mid_y), dx_clear, (dx, dy) ]
        if not path_hits_any(vh, obstacles): return vh

        # Tertiary: route around right side
        gap = 30
        far_x = max(self.src.x + self.src.w, self.dst.x + self.dst.w) + gap
        via_r = [ (sx, sy), sx_clear, (far_x, sx_clear[1]), (far_x, dx_clear[1]), dx_clear, (dx, dy) ]
        if not path_hits_any(via_r, obstacles): return via_r

        # Fallback: simple L-shape using clear points
        simple = [ (sx, sy), sx_clear, (sx_clear[0], dx_clear[1]), dx_clear, (dx, dy) ]
        return simple
...
class Context:
    def __init__(self, font, small, big):
        ...
        bx = 1440; by = 100
        self.btn_toggle = Button(bx, by, 140, 34, "▶ Play (Space)")
        self.btn_step   = Button(bx, by+48, 140, 34, "⏩ Step (N)")
        self.btn_reset  = Button(bx, by+96, 140, 34, "↺ Reset (R)")
        self.btn_full   = Button(bx, by+144, 140, 34, "⛶ Fullscreen (F11)")
    ...
    def draw(self, screen):
        ...
        screen.blit(self.big.render("ENIAC Ballistics – OrCAD Layout (toggle)", True, TEXT), (28, 18))
        self._draw_hud(screen); self._draw_buttons(screen)
    ...
    def _draw_buttons(self, s):
        # toggle label
        self.btn_toggle.label = "⏸ Pause (Space)" if self.running else "▶ Play (Space)"
        for b in (self.btn_toggle, self.btn_step, self.btn_reset, self.btn_full):
            b.draw(s, self.font)

    def on_click(self, pos):
        if self.btn_toggle.hit(pos): self.toggle()
...
def main():
    ...
    pygame.display.set_caption("ENIAC Ballistics – OrCAD Layout (toggle)")
    ...
    while running:
        ...
        for e in pygame.event.get():
            if e.type == pygame.QUIT:
                ...
                elif e.key == pygame.K_SPACE: ctx.toggle()
        ...
        ctx.update(dt)
        screen.fill(BG); ctx.draw(screen); pygame.display.flip()
